...
public class SVMTrain {
	...
	private void getPlateTest(MatVector testingImages,
	...
	public void getHasPlateTest(MatVector testingImages,
			Vector<Integer> testingLabels) {
		getPlateTest(testingImages, testingLabels, hasPlate);
	}

	public void getNoPlateTest(MatVector testingImages,
			Vector<Integer> testingLabels) {
	...
	public int svmTrain(boolean dividePrepared, boolean trainPrepared) {
		...
		if (!trainPrepared && !classes.empty() && !trainingData.empty()) {
			CvSVMParams SVM_params = new CvSVMParams(CvSVM.C_SVC, CvSVM.RBF,
					0.1, 1, 0.1, 1, 0.1, 0.1, new CvMat(), new CvTermCriteria()
							.type(CV_TERMCRIT_ITER).max_iter(100000)
							.epsilon(0.0001));
			// Train SVM
			System.out.println("Begin to generate svm");
			try {
				// CvSVM svm(trainingData, classes, Mat(), Mat(), SVM_params);
				svm.train_auto(trainingData, classes, new Mat(), new Mat(),
						SVM_params, 10, CvSVM.get_default_grid(CvSVM.C),
			...
			CvFileStorage fsTo = CvFileStorage.open("res/rain/svm.xml",
