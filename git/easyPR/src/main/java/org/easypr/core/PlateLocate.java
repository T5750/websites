...
public class PlateLocate {
	...
	public Vector<Mat> plateLocate(Mat src) {
		...
		GaussianBlur(src, src_blur,
				new Size(gaussianBlurSize, gaussianBlurSize), 0, 0,
				BORDER_DEFAULT);
		if (debug) {
		...
		Mat element = getStructuringElement(MORPH_RECT, new Size(
				morphSizeWidth, morphSizeHeight));
		...
		for (int i = 0; i < rects.size(); i++) {
			...
			if (verifySizes(minRect)) {
				if (debug) {
					...
					for (int j = 0; j < 4; j++) {
						Point pt1 = new Point(rect_points.position(j)
								.asCvPoint2D32f());
						Point pt2 = new Point(rect_points.position((j + 1) % 4)
								.asCvPoint2D32f());
						line(result, pt1, pt2, new Scalar(0, 255, 255, 255), 1,
								8, 0);
					}
				}
