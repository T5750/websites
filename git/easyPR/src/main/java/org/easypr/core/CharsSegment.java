...
public class CharsSegment {
	...
	public int charsSegment(final Mat input, Vector<Mat> resultVec) {
		...
		int w = input.cols();
		int h = input.rows();
		Mat tmpMat = new Mat(input, new Rect((int) (w * 0.1), (int) (h * 0.1),
				(int) (w * 0.8), (int) (h * 0.8)));
		...
		for (int i = 0; i < newSortedRect.size(); i++) {
			...
			if (this.isDebug) {
				String str = "tmp/debug_char_auxRoi_"
	...
	private Boolean verifySizes(Mat r) {
		...
		return percPixels <= 1 && charAspect > minAspect
				&& charAspect < maxAspect && r.rows() >= minHeight
				&& r.rows() < maxHeight;
	}
