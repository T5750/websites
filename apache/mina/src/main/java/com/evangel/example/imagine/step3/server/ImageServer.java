 ...
 *
 */
package com.evangel.example.imagine.step3.server;
...
import com.evangel.example.imagine.step1.codec.ImageCodecFactory;
...
public class ImageServer {
	...
	public static void main(String[] args) throws Exception {
		...
		// register the bean on the MBeanServer. Without this line, no JMX will
		// happen for
		// this acceptor.
		mBeanServer.registerMBean(acceptorMBean, acceptorName);
		// add an IoFilter . This class is responsible for converting the
		// incoming and
		// outgoing raw data to ImageRequest and ImageResponse objects
		ProtocolCodecFilter protocolFilter = new ProtocolCodecFilter(
				new ImageCodecFactory(false));
		...
		ObjectName protocolFilterName = new ObjectName(protocolFilter
				.getClass().getPackage().getName()
				+ ":type=protocolfilter,name="
				+ protocolFilter.getClass().getSimpleName());
		// register the bean on the MBeanServer. Without this line, no JMX will
		// happen for
		// this filter.
		mBeanServer.registerMBean(protocolFilterMBean, protocolFilterName);
		// add the protocolFilter to the acceptor, otherwise no filtering of
		// data will happen
		...
		DefaultIoFilterChainBuilder filterChainBuilder = acceptor
				.getFilterChain();
		// add an ExecutorFilter to the filter chain. The preferred order is to
		// put the executor filter
		// after any protocol filters due to the fact that protocol codecs are
		// generally CPU-bound
