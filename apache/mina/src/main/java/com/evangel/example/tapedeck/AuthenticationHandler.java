 ...
 *
 */
package com.evangel.example.tapedeck;
...
import com.evangel.example.tapedeck.TapeDeckServer.TapeDeckContext;
...
public class AuthenticationHandler {
	...
	public void password(AuthenticationContext context, NextFilter nextFilter,
		...
		if (context.password.equals(reverse(context.user))) {
			...
			if (context.tries < 3) {
				...
				session.write(
						"- Authentication failed too many times! Bye bye.")
	...
	public void errorWaitingForUser(IoSession session, Command cmd) {
		...
		session.write("Unexpected command '" + cmd.getName()
