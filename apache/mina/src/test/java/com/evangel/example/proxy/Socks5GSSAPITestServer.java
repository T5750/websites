...
/**
 * Socks5GSSAPITestServer.java - Basic test server for SOCKS5 GSSAPI
 * authentication.
 * 
 * NOTE: Launch this program with the following params in a pre-configured
 * Kerberos V env. Do not forget to replace < ... > vars with your own values.
 * 
 * -Djava.security.krb5.realm=<your_krb_realm>
 * -Djavax.security.auth.useSubjectCredsOnly=false
 * -Djava.security.krb5.kdc=<your_kdc_hostname>
 * -Djava.security.auth.login.config
 * =${workspace_loc}\Mina2Proxy\src\bcsLogin.conf -Dsun.security.krb5.debug=true
 * 
...
public class Socks5GSSAPITestServer {
	...
	/**
	 * Selected mechanism message: advertises client to use SocksV5 protocol
	 ...
	 * 
	 * @param localPort
	 *            the local port used to bind the server
	 * @throws IOException
	 * @throws GSSException
	 */
				...
				while (!context.isEstablished()) {
					...
					if (token != null) {
						logger.debug("    Sending Token[{}] = {}",
								token.length, ByteUtilities.asHex(token));
