 ...
 * under the License.
 */
package com.evangel.example.haiku;
...
public class ToHaikuIoFilterTest extends MockObjectTestCase {
	...
	public void testThreeStringsMakesAHaiku() throws Exception {
		...
		list.expects(once()).method("add").with(eq("two"))
				.will(returnValue(true));
		list.expects(once()).method("add").with(eq("three"))
				.will(returnValue(true));
		...
		session.expects(exactly(3))
				.method("getAttribute")
				.with(eq("phrases"))
				.will(onConsecutiveCalls(returnValue(null),
		...
		session.expects(exactly(1)).method("removeAttribute")
				.with(eq("phrases"));
