 ...
 *
 */
package com.evangel.example.proxy;
...
public class ProxyTestClient {
	...
	/**
	 * Creates a connection to the endpoint through a proxy server using the
	 * specified authentication method.
	 * 
	 * Command line arguments: ProxyTestClient <proxy-hostname> <proxy-port>
	 * <url> <proxy-method>
	 * 
	 * Note that <proxy-method> is OPTIONNAL a HTTP proxy connection will be
	 * used if not specified.
	 * 
	 * Examples: ProxyTestClient myproxy 8080 http://mina.apache.org SOCKS4
	 * ProxyTestClient squidsrv 3128 http://mina.apache.org:80
	 * 
	 * @param args
	 *            parse arguments to get proxy hostaname, proxy port, the url to
	 *            connect to and optionnaly the proxy authentication method
	 * @throws Exception
	 */
	...
	/**
	 * Creates a {@link HttpProxyRequest} from the provided <i>uri</i>
	 * parameter. It uses the global variables defined at the top of the class
	 * to fill the connection properties of the request. If the global variable
	 * <i>useHttp1_1</i> is set to true, it will create a HTTP/1.1 request.
	 * 
	 * @param uri
	 *            the requested uri to connect to through the HTTP proxy
