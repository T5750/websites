...
public class TransactionProducer {
	...
	public static void main(String[] args) throws MQClientException,
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			private void printStats() {
				if (snapshotList.size() >= 10) {
					...
					System.out
							.printf("Send TPS: %d Max RT: %d Average RT: %7.3f Send Failed: %d Response Failed: %d transaction checkCount: %d %n",
									sendTps, statsBenchmark
											.getSendMessageMaxRT().get(),
									averageRT, end[2], end[4], end[6]);
				}
			}
		...
		final TransactionCheckListener transactionCheckListener = new TransactionCheckListenerBImpl(
				ischeckffalse, statsBenchmark);
		...
		producer.start();
		final TransactionExecuterBImpl tranExecuter = new TransactionExecuterBImpl(
				ischeck);
	...
	private static Message buildMessage(final int messageSize)
		...
		msg.setTopic(Globals.TOPIC_BENCHMARK);
